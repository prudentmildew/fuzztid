// The app's clock utilities, source-agnostic and injected: every caller
// passes its own `now: Date` rather than reading the system clock directly,
// so a test pins time by argument (#23).
...
/** A Day's relation to the real-world Oslo date (ADR-0022, ported by reference). */
export type DayStanding = "past" | "today" | "future";

/**
 * Day standing plus the dormant state: `"none"` means no pane is today, which
 * is the case for all but the two Høstsabbat days.
 */
